...
namespace repost
{
    ...
    remote_client::remote_client(tcp::socket socket, message_handler& messageHandler)
        : _socket(std::move(socket))
        , _messageHandler(messageHandler)
    {
    }

    void remote_client::start()
    {
        do_read_header();
    }

    void remote_client::deliver(const std::string& msg)
    {
    ...
    void remote_client::do_read_header()
    {
    ...
    void remote_client::do_read_body()
    {
        auto self(shared_from_this());
        boost::asio::async_read(_socket,
